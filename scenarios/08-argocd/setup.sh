...
# in-cluster git server (seeded with scenarios/08-argocd/app/), then creates an ArgoCD Application that
...
#
# This uses the k8store + PostgreSQL setup (scenario 4) as the example. The Keycloak image is
# scenario 4's, so it is built here if missing (CI builds it in a real setup, where ArgoCD only
...
require_cluster
KC_IMAGE="${CNK_REGISTRY}/cnk-04:dev"
GIT_IMAGE="${CNK_REGISTRY}/cnk-git-server:dev"
ARGOCD_VERSION="${ARGOCD_VERSION:-stable}"

# 1. Keycloak image (scenario 4's). Build if absent.
if ! docker image inspect "${KC_IMAGE}" >/dev/null 2>&1; then
  log "Building the Keycloak image (scenario 4)"
  ( cd scenarios/04-k8store-postgres && ./build-providers.sh && docker build -q -t "${KC_IMAGE}" . )
fi
docker push -q "${KC_IMAGE}" >/dev/null

# 2. In-cluster git server image, seeded with scenarios/08-argocd/app/
log "Building and pushing the in-cluster git server (seeded with scenarios/08-argocd/app/)"
docker build -q -f scenarios/08-argocd/git-server/Dockerfile -t "${GIT_IMAGE}" scenarios/08-argocd/ >/dev/null
...
${KUBECTL} apply -f scenarios/08-argocd/git-server/git-server.yaml >/dev/null
...
${KUBECTL} apply -f scenarios/08-argocd/application.yaml >/dev/null
...
ArgoCD deployed Keycloak + CRs from the in-cluster git repo (read-only).
  ...
  Change config the GitOps way: push a commit to the git server (see scenarios/08-argocd/README.md), and
  ArgoCD reconciles it. No image rebuild.
  Tear down: scenarios/08-argocd/teardown.sh
EOF
