# Scenario 5 — filestore (config in YAML files) + Cassandra (dynamic)
...
database, see [scenario 4](../04-filestore-postgres)).
...
Like [scenario 4](../04-filestore-postgres), filestore config is per-pod:
...
The same demo config as [scenario 4](../04-filestore-postgres) (the `master` and `demo` realms as
